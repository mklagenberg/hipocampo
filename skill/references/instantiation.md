...
2. **Collect the `inputs` declared in the profile** directly from the user — repository name, `tier` (`confidencial` or `público`, decision 0029), the owner's identity (full name + `@github-handle` in the `pessoal` domain; legal company name in the `empresa` domain), and the vault's content language (`language` input, defaults to `en` — confirm with the user rather than silently assuming the default, decision 0034). Never assume a value the profile marks as required.
...
>
> Agent: profile `empresa.yaml`, tier to ask about ("confidencial or público?"). User: "confidencial". The agent collects the legal company name, confirms the vault's content language rather than assuming the English default — user says "pt-BR" — then presents the plan (repo name, filled-in `LICENSE-corporativo`, `AGENTS.md` with "Instance type: corporativa", `hipocampo.yaml` with `domain: empresa`/`tier: confidencial`/`language: pt-BR`), waits for confirmation, creates the private repository and the files, then points to the generated `POST-INSTANTIATION.md` for final review.