...
- **v2.1.0 consistency contracts and Codex distribution** (`changes/0052-consistency-contracts-and-codex-distribution`) — anchor manifests can register operator-confirmed repository addresses without restoring a client-side router; target manifests remain authoritative for entity, role, and scope. New manifests use `instance.policy_profile` for sensitive-data policy and `instance.curation_level` for repository curation, resolving the former `tier` collision while preserving read compatibility with v2 manifests. The canonical skill is 1.2.0, includes invariant 6 and a declared Codex adapter, and a new CI validator checks these contracts across specification, scaffold, skill, and disclosures.
