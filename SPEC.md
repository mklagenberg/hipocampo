...
**License boundary.** This repository's Apache-2.0 `LICENSE` applies only to the methodology and its tooling. A content-bearing vault has its own strictly private proprietary root `LICENSE`, generated from the scaffold for its owner and curation level. A document's `visibility` classification never grants an open license, external publication, copying, or redistribution. Neither license text nor a generic privacy-policy file is copied into a non-content repository, a client-side skill, or host-local adapter state; see `docs/privacy-and-licensing-boundaries.md`.
...
### visibility
Resolves **only** the handling limit among people already authorized to access the private repository: `public` (no additional handling restriction within that authorized context), `internal` (ordinary internal handling), `confidential` (need-to-know handling even within the repository), `restricted` (individualized handling, case by case). `visibility` **never** grants an open license, external publication, copying, redistribution, or exposure to the internet — all knowledge repositories remain private by invariant, section 8. A `confidential` label in a repository the whole team accesses does not stop anyone on that team from reading the file — real GitHub permission is repository-level granularity, not file-level granularity within a shared repository. Content that actually needs technical enforcement goes into a separate repository with restricted access permission, not into a `visibility` label inside a broader-access repository.
...
### curation_status
Optional field, relevant only within a restricted corporate vault (section 2-C — formerly described as `empresa-confidencial`, see `docs/vocabulary-dictionary.md`). Flags the intended lifecycle of the document within that repository: `staged` marks a candidate for eventual promotion to a broader-access private corporate vault, after leadership curation; `permanent` (default, when the field is left empty) marks content confidential by nature, with no expectation of broader authorized use. It does not replace or overlap with `visibility` — the two fields resolve different questions: `visibility` is about handling among people already authorized to access the private repository; `curation_status` is about whether that specific document is a candidate to change repository someday. See `decisions/0029-repository-type-taxonomy.md`.

### license
Always mechanically derived from `visibility`, never set by hand — this avoids divergence between the handling layer (`visibility`) and its repository-level governance reference (`license`). Uses the SPDX pattern `LicenseRef-<idstring>`, with the full legal text in the strictly private proprietary `LICENSE` file at the repository root, never rewritten per document. A `LicenseRef` is not an open license and cannot grant rights beyond that root contract. See `decisions/0007-content-repo-licensing.md` and `decisions/0054-private-proprietary-content-license-boundary.md`.
...
Every entity has exactly one mandatory **anchor** vault (private) and may have any number of **additional** private vaults with a declared scope — this is normal multi-vault operation, not an edge case (section 2-D, premise 3). Each vault self-declares, in its own manifest (`hipocampo.yaml`, `decisions/0033`/`0041`):
...
The `confidential`/`public` **visibility classifications** are unchanged as controlled values, but no longer imply a public repository or an open license. A repository-name `restricted`/`open` suffix, if an operator uses one, describes its relative access posture (e.g. `<entity>-restricted-vault`, `<entity>-open-vault`) and does not alter the mandatory private/proprietary boundary. There is still no third "structuring" tier as a separate repository: confidential knowledge that is a candidate for broader authorized use keeps living in the confidential vault, with the lifecycle intent marked in the document's own frontmatter (`curation_status`, section 2), not by an additional physical separation — this reasoning, from `decisions/0029`, is unchanged by the entity model.

The formal declaration of which entity+role a specific vault implements is operationalized by the instance manifest (`hipocampo.yaml`, `decisions/0033`/`0041`). `AGENTS.md`'s legacy "instance type" field (`corporate`/`personal`, section 2-A/11) is recommended for retirement, not yet removed from any real instance. `instance.policy_profile`, not `entity`+`role` or a duplicate `AGENTS.md` value, selects the sensitive-data policy; keeping a manually filled echo would reintroduce the divergence risk this schema change resolves. See `decisions/0052` for the policy-selector rationale and `decisions/0041` for migration status.
...
2. **Confidential-first, always.** Information is confidential by default; promotion to a broader-access private vault is always a later, explicit step (the Promote action, section 13) — with no exception for content that looks "obviously public." There is never a direct write path that grants external publication or an open license.
...
**Graduation within the same domain (new, `decisions/0030`):** Promote also covers the case where a restricted corporate document marked `curation_status: staged` (section 2-C) is ready to become a broader-access private corporate document — same ownership domain the whole time, so it always uses the elegant path (never the literal one, since there's no new ownership transfer at stake, `decisions/0007` doesn't change in this case). The source document is not deleted; `curation_status` either becomes `permanent` (ending its candidacy) or the document ends up with `related` pointing to the new destination document, at the discretion of whoever confirms the action. Only a `staged` document is eligible for this variant — a `permanent` document needs an explicit `curation_status` reclassification first, a human decision separate from the promotion itself.

### Depromote — moving down within the same ownership domain

Moves content between repositories of the same owner (e.g., a broader-access private corporate vault → a restricted corporate vault, or between personal variants), without crossing the personal/corporate boundary — so it doesn't carry the ownership question of the literal Promote, and doesn't need the equivalent explicit warning. Mechanics: `status: superseded` at the source, `superseded_by: $alias:destination`. A literal reversal of Promote (corporate → personal, crossing the ownership boundary back) is out of scope for this action — it is not automated; it's a case-by-case decision by the responsible human, outside Hipocampo's normal flow, in the same spirit as `DISCLAIMER.md` ("does not replace legal compliance").
