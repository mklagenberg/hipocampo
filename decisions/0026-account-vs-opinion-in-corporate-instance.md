...
- **Keep this decision's original account/opinion binary.** Rejected: the corpus itself already demonstrates the need for a third state (confirmed fact, see `modelo-matricial-acme-e-papel-do-principal.md`) and a fourth categorically distinct one (personal memory, reconstructive by nature, different from a third party's account and from a value judgment).
