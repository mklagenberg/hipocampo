...
Concretely exercised with a hypothetical case (4 repositories stuck on v1.3.0, methodology at v1.9.0 + unreleased) and confirmed against a real case: the `hipocampo-company-vault` (Gauge) repository is in exactly this situation — skill still at the "stub" stage (prior to v1.7.0), `CLAUDE.md` never migrated to `AGENTS.md`, `LICENSE` not verified. No one had a checklist to apply against it.
