...
No file in the `hipocampo` methodology repository may name a real, identifiable third-party entity — a company, business unit, or named individual — nor a real, identifiable personal account handle belonging to a third party, in any context: worked example, evaluation scenario, Decision Record rationale, audit finding, or Change Set note. This applies unconditionally, with no risk-based exception — a single company name in one worked example is not judged "low-risk enough" to keep. A necessary provenance reference to the repository's own operator is outside this policy, but it must not disclose a third party's identity.
