...
None released yet, but the unreleased work on `main` toward v2.0.0 already includes at least one breaking schema change (`hipocampo.yaml`'s `instance.domain` → `instance.entity`/`instance.role`, `decisions/0041`) — see `MIGRATIONS.md`, "1.x → 2.0", for the accumulating migration steps. Per `decisions/0023-operational-criterion-for-semver-scope.md`, the mandatory migration steps for a MAJOR change go into `MIGRATIONS.md`, not this file.
